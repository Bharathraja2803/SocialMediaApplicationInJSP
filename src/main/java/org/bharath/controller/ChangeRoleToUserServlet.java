...
/**
 * Servlet implementation class ChangeRoleToUserServlet
 */
public class ChangeRoleToUserServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean isRoleUpdated = usersDaoImpl.updateTheRoleOfTheUser(users.getUserId_(), targetUserId, "user");
		
		if(!isRoleUpdated){
			MainCentralizedResource.LOGGER.warning("Something went wrong in updating the admin role to user");
			session.setAttribute("isRoleUpdateFailure", "y");
			response.sendRedirect("allUsersList.jsp");
			return;
		}
		
		MainCentralizedResource.LOGGER.info("Role of the admin changes to user");
